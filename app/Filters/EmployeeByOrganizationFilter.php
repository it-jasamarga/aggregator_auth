...
class EmployeeByOrganizationFilter extends QueryFilters
{
    protected $request;
    ...
    public function OrganizationID($term) {
        return $this->builder->whereHas('position', function($q) use($term){
            $q->where('active','1')->whereHas('masterPosition',function($q1) use($term){
                $q1->where('org_id',$term);
            });
        });
    }
